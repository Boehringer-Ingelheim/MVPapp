...
A list of models (stored as character objects) are included by default when `MVPapp` is installed, from  `R/code_templates.R`. This is the list that the users sees in the drop-down list when they select models, and is stored as a vector of characters called `model_examples_list`. There are two functions related to model selection:  
...
An example external file is provided in `inst/shiny/passworded_models_example.R`. The user is recommended to copy this script locally and use it as a starting point to insert models, and supply the path for the `pw_models_path` argument to `run_mvp()`, i.e. `run_mvp(pw_models_path = "path/to/your/private/models.R")`.

## Re-defining Model Switch Conditions
...
## Re-defining Update Model Choices
...
If the user wishes to update other Inputs that are not listed above, please refer to the source code for `"inst/shiny/app.R"`.  
